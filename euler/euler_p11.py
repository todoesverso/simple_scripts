...
for y in range(0, columns ):
    for j in range(0, rows ):
        # Start asking for the indexes
        
        # \
        if y <= (columns - 4) and j <= (rows - 4):
            value2 = matrix[y][j] * matrix[y+1][j+1] * matrix[y+2][j+2] * matrix[y+3][j+3]
        # -->
        if j <= (columns - 4):
            value1 = matrix[y][j] * matrix[y][j+1] * matrix[y][j+2] * matrix[y][j+3]
        # |
        if y <= (rows - 4):
            value3 = matrix[y][j] * matrix[y+1][j] * matrix[y+2][j] * matrix[y+3][j]
        # /    
        if y >= (4) and j <= (rows - 4):
            value4 = matrix[y][j] * matrix[y-1][j+1] * matrix[y-2][j+2] * matrix[y-3][j+3]
